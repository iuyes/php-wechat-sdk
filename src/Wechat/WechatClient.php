...
class WechatClient {
    ...
    function __construct($appid = '', $appsecret = '')
    {
        ...
        {
            $this->_appid     = $this->config->read('wechat/account/appid');
            $this->_appsecret =  $this->config->read('wechat/account/appsecret');
        }
    }
    ...
    public function getAccessToken($tokenOnly = 1, $nocache = 0)
    {
        ...
        {
            self::$_accessTokenCache[$appid] = $this->cache->get($cachename);
        }
        ...
        if (self::checkIsSuc($res))
        {
           ...
           $this->cache->set($cachename, $myTokenInfo, ((int)$res['expires_in']));
        }
    ...
    private function _getJSticket($ticketOnly = 1, $nocache = 0)
    {
        ...
        if ($nocache || empty(self::$_accessTokenCache[$appid]))
        {
            self::$_jsapiTicketCache[$appid] = $this->cache->get($cachename);
        }
        ...
        if (self::checkIsSuc($res))
        {
            ...
            $this->cache->set($cachename, $jsTicketInfo, ((int)$res['expires_in']));
        }
     ...
     * @return bool
     */
    public function sendVoiceMsg($to, $mid)
    {
     ...
     * @return bool
     */
    public function sendVideoMsg($to, $mid, $title, $desc)
    {
     ...
     * @return bool
     */
    public function sendMusicMsg($to, $url, $mid, $thumb_mid, $title, $desc = '', $hq_url = '')
    {
